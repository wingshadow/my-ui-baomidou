<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="初领日期" prop="firstLicenseDate">
        <el-date-picker clearable
          v-model="queryParams.firstLicenseDate"
          type="date"
    ...
    <el-table v-loading="loading" :data="examineList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="考试主键" align="center" prop="examineId" v-if="true"/>
      ...
      <el-table-column label="初领日期" align="center" prop="firstLicenseDate" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.firstLicenseDate, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="准驾车型" align="center" prop="vehicleType" />
...
<script>
import { listExamine, getExamine, delExamine, addExamine, updateExamine } from "@/api/biz/examine";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        examineId: [
          { required: true, message: "考试主键不能为空", trigger: "blur" }
        ],
        userId: [
        ...
        birthday: [
          { required: true, message: "出生日期;yyyy-mm-dd不能为空", trigger: "blur" }
        ],
        education: [
        ...
        firstLicenseDate: [
          { required: true, message: "初领日期不能为空", trigger: "blur" }
        ],
        vehicleType: [
          { required: true, message: "准驾车型不能为空", trigger: "change" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除【请填写功能名称】编号为"' + examineIds + '"的数据项？').then(() => {
        this.loading = true;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/examine/export', {
        ...this.queryParams
